...
| 공정성 | 당첨은 [정지!] 순간 **칸 수 비율대로 무작위 추첨** — 룰렛 각도·누른 시각과 무관해 타이밍으로 고를 수 없다. 당첨 확률을 도장과 기록에 표시 |

**모집 마감 카운트다운**: [⏱ 30초 후 마감]은 **메뉴 접수를 닫는 버튼**이다. 누르면 룰렛 중앙에 카운트다운이 뜨고, 그동안 들어온 도네까지만 반영된다. 시간이 끝나도 자동으로 돌지는 않으며 [돌리기!]를 눌러야 시작한다. 버튼을 다시 누르면 연장되고, 마감된 뒤에 눌러도 다시 받는다(횟수 제한 없음).
...
방송 화면 연출: 메뉴가 붙으면 룰렛 위에 토스트, 리롤권이 쌓이면 구매자 이름을 크게 표시, 연결이 끊기면 상단에 빨간 경고 띠와 경보음.
