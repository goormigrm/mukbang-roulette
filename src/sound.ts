...
// ---- 회전 중 배경음 — "감정가 발표 직전" 느낌의 긴장 오스티나토 (자작 합성, 원곡 사용 아님) ----
// 낮게 고동치는 D단조 베이스 8분음표 + 반음(Eb) 긴장 + 서서히 열리는 단조 패드 + 박마다 낮은 팀파니.
// 마디가 지날수록 패드 필터가 열리고 커져 "곧 결과가 나온다"는 빌드업이 된다.
const BGM_BPM = 104
const BGM_STEP_SEC = 60 / BGM_BPM / 2 // 8분음표 하나
// 2마디(16스텝) 베이스 라인 — D2 고동, 7번째에 Eb2로 찔러 긴장, 둘째 마디 끝에 C→A→Bb로 가라앉음
const BGM_BASS = [73.42, 73.42, 73.42, 73.42, 73.42, 73.42, 77.78, 73.42, 73.42, 73.42, 73.42, 73.42, 65.41, 65.41, 55.0, 58.27]
const BGM_PAD = [146.83, 174.61, 220.0] // D3 F3 A3 (D단조)
const BGM_BASE_GAIN = 0.9
...
function bgmStep(a: AudioContext, b: Bgm, i: number, t: number): void {
  // 베이스 — 짧게 끊어 치는 톱니파 + 로우패스 (고동)
  const bass = a.createOscillator()
  bass.type = 'sawtooth'
  bass.frequency.value = BGM_BASS[i]
  const lp = a.createBiquadFilter()
  lp.type = 'lowpass'
  lp.frequency.value = 360
  const g = a.createGain()
  g.gain.setValueAtTime(0.0001, t)
  g.gain.exponentialRampToValueAtTime(0.16 * MASTER, t + 0.012)
  g.gain.exponentialRampToValueAtTime(0.0001, t + BGM_STEP_SEC * 0.75)
  bass.connect(lp).connect(g).connect(b.gain)
  bass.start(t)
  bass.stop(t + BGM_STEP_SEC)

  // 매 스텝 아주 작은 시계 틱 — 초조함
  const tk = a.createBufferSource()
  tk.buffer = noise(a)
  const tf = a.createBiquadFilter()
  tf.type = 'bandpass'
  tf.frequency.value = 3600
  const tg = a.createGain()
  tg.gain.setValueAtTime((i % 2 === 0 ? 0.035 : 0.02) * MASTER, t)
  tg.gain.exponentialRampToValueAtTime(0.0001, t + 0.03)
  tk.connect(tf).connect(tg).connect(b.gain)
  tk.start(t)
  tk.stop(t + 0.04)

  // 마디 첫 박 — 낮은 팀파니 (음정이 살짝 떨어지는 사인파)
  if (i % 8 === 0) {
    const tim = a.createOscillator()
    tim.type = 'sine'
    tim.frequency.setValueAtTime(62, t)
    tim.frequency.exponentialRampToValueAtTime(44, t + 0.45)
    const tgn = a.createGain()
    tgn.gain.setValueAtTime(0.32 * MASTER, t)
    tgn.gain.exponentialRampToValueAtTime(0.0001, t + 0.55)
    tim.connect(tgn).connect(b.gain)
    tim.start(t)
    tim.stop(t + 0.6)
    const th = a.createBufferSource() // 타격감
    th.buffer = noise(a)
    const thf = a.createBiquadFilter()
    thf.type = 'lowpass'
    thf.frequency.value = 500
    const thg = a.createGain()
    thg.gain.setValueAtTime(0.14 * MASTER, t)
    thg.gain.exponentialRampToValueAtTime(0.0001, t + 0.12)
    th.connect(thf).connect(thg).connect(b.gain)
    th.start(t)
    th.stop(t + 0.15)
  }

  // 새 2마디마다 패드가 조금씩 더 열리고 커진다 (빌드업, 8회까지)
  if (i === 0) {
    b.bar++
    const k = Math.min(1, b.bar / 8)
    b.padFilter.frequency.setTargetAtTime(380 + 1600 * k, t, 0.6)
    b.padGain.gain.setTargetAtTime((0.05 + 0.07 * k) * MASTER, t, 0.6)
  }
}

/** 회전 시작과 함께 배경음 시작 (이미 켜져 있으면 유지) */
export function startBgm(): void {
  ...
  // 아주 조용한 고음 반음(Eb5) — 불안감
  const tense = a.createOscillator()
  tense.type = 'sine'
  tense.frequency.value = 622.25
  const tenseGain = a.createGain()
  tenseGain.gain.value = 0.012 * MASTER
  tense.connect(tenseGain).connect(gain)
  tense.start()
  oscs.push(tense)

  const b: Bgm = { gain, padFilter, padGain, oscs, timer: 0 as unknown as ReturnType<typeof setInterval>, nextTime: a.currentTime + 0.05, step: 0, bar: 0 }
  b.timer = setInterval(() => {
    // 0.25초 앞까지 미리 예약 (setInterval 지터에 흔들리지 않게)
    while (b.nextTime < a.currentTime + 0.25) {
      bgmStep(a, b, b.step, b.nextTime)
      b.step = (b.step + 1) % BGM_BASS.length
      b.nextTime += BGM_STEP_SEC
    }
  }, 60)
  bgm = b
}

/** 배경음 상대 음량 (0~1) — 드럼롤 구간에서 한 발 물러날 때 */
export function duckBgm(ratio: number): void {
  const a = ac()
  if (!a || !bgm) return
  bgm.gain.gain.setTargetAtTime(BGM_BASE_GAIN * ratio, a.currentTime, 0.25)
}

/** 배경음 정지 — 0.6초에 걸쳐 사라진다 */
export function stopBgm(): void {
  ...
  b.gain.gain.setTargetAtTime(0, t, 0.18)
