...
| 같은 메뉴 | 띄어쓰기 차이와 뒤에 붙은 "추가"는 무시하고 **먼저 올라온 항목에 합산** (`부대 찌개`·`부대찌개 추가` → `부대찌개`) |
...
**버저비터**: 방송은 실시간보다 몇 초 늦게 나가므로, 카운트다운이 0이 된 뒤 **5초**(설정에서 변경, 0이면 끔) 동안 도착한 도네까지는 그대로 반영한다. 그 뒤에 온 도네는 피드에만 남고, 넣어줄지는 [메뉴 직접 추가]로 진행자가 정한다.
...
**이번 판의 이름**은 진행 중 탭 [오늘의 후보] 제목 오른쪽 칸에 적는다(메인 메뉴 / 사이드 / 디저트 등) — 적는 즉시 룰렛 오른쪽 위에 현판으로 뜨고, 기록에도 `🏷 이름` 태그로 함께 남는다(안 적은 판은 공란).
